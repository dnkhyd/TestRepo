	         ...
	         <?php 
	         	for ($i=0;$i<sizeof($imageSettings);$i++){
	         		if($imageSettings[$i]['active']==1){
	  			     	echo '<input id="'.$imageSettings[$i]['setting_name'].'" type="radio" name="orderGroup" checked="true" onClick="updateImageDisplayOrder(id);">'.$imageSettings[$i]['setting_name'];
	         		}else{
	         			echo '<input id="'.$imageSettings[$i]['setting_name'].'" type="radio" name="orderGroup" onClick="updateImageDisplayOrder(id);">'.$imageSettings[$i]['setting_name'];
	         		}
			 	}
			 ?>
		 </div>
      ...
      <?php  } else {?>
		...
		if(sizeof($images)>0){
			...
			for($i=0;$i<sizeof($images);$i++){
				...
				echo "<span onmouseover=ShowPopup('".$images[$i]['img_src']."'); onMouseOut=HidePopup(); style='cursor: pointer;'>".htmlentities($images[$i]['img_title'])."</span>";
				echo " </td>";
				...
				if (!empty($desc)){
					if($images[$i]['img_desc']!=''){
						echo "<td  width='13%' class='blu_txt' align='center' style='border-bottom:1px solid #aaaaaa;height:50px;'>";
						echo "<span onmouseover=\"ddrivetip(&#39;".htmlentities($images[$i]['img_desc'])."&#39;,'#ffffff','300')\"; onMouseOut=hideddrivetip(); style='cursor: pointer;'><img src='http://".ROOT."/images/des.png'></img></span>";
						echo " </td>";
					}
				}else{
					echo "<td  width='13%' class='blu_txt' align='center' style='border-bottom:1px solid #aaaaaa;height:50px;'>";
					echo "<span onmouseover=\"ddrivetip(&#39;No description. &#39;,'#ffffff','300')\"; onMouseOut=hideddrivetip(); style='cursor: pointer;'><img src='http://".ROOT."/images/des.png'></img></span>";
					echo " </td>";
				}
				if($status==1){
					echo "<td  class='blu_txt'width='13%' align='center' style='border-bottom:1px solid #aaaaaa;'>".$images[$i]['img_order']."</td>";
				 	echo "<td  class='blu_txt'width='13%' align='center' style='border-bottom:1px solid #aaaaaa;' ><img src='http://".ROOT."/images/edit.png' onclick=\"editImages(".$id.",&#39;".$desc."&#39;,&#39;".htmlentities($images[$i]['img_title'])."&#39;)\"  style='cursor: pointer;'></img></td>";
