...
function network_step1( $errors = false ) {
	...
	if ( isset( $_POST['subdomain_install'] ) ) {
		...
		if ( $got_mod_rewrite || $is_apache ) // Protect against mod_rewrite mimicry (but ! Apache)
			echo '<p>' . __( 'If <code>mod_rewrite</code> is disabled, ask your administrator to enable that module, or look at the <a href="httpd.apache.org/docs/mod/mod_rewrite.html">Apache documentation</a> or <a href="http://www.google.com/search?q=apache+mod_rewrite">elsewhere</a> for help setting it up.' ) . '</p></div>';
	}
    ...
    <system.webServer>
        <rewrite>
            <rules>
                ...
                <rule name="WordPress Rule 3" stopProcessing="true">
                    <match url="^([_0-9a-zA-Z-]+/)?blogAdmin$" ignoreCase="false" />
                    <action type="Redirect" url="{R:1}blogAdmin/" redirectType="Permanent" />
                </rule>
                <rule name="WordPress Rule 4" stopProcessing="true">
                    <match url="^" ignoreCase="false" />
                    <conditions logicalGrouping="MatchAny">
...
RewriteRule ^' . ( $subdomain_install ? '' : '([_0-9a-zA-Z-]+/)?' ) . 'files/(.+) wp-includes/ms-files.php?file=$' . ( $subdomain_install ? 1 : 2 ) . ' [L]' . "\n";
		...
		if ( ! $subdomain_install )
			$htaccess_file .= "\n# add a trailing slash to /blogAdmin\n" . 'RewriteRule ^([_0-9a-zA-Z-]+/)?blogAdmin$ $1blogAdmin/ [R=301,L]' . "\n";
...
if ( $_POST ) {
	...
	require_once( ABSPATH . 'blogAdmin/includes/upgrade.php' );
