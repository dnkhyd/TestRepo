...
require_once(ABSPATH . 'blogAdmin/includes/admin.php');
...
require(ABSPATH . 'blogAdmin/menu.php');
...
if ( isset($plugin_page) ) {
	if ( $page_hook ) {
		...
		if (! isset($_GET['noheader']))
			require_once(ABSPATH . 'blogAdmin/admin-header.php');
	...
	include(ABSPATH . 'blogAdmin/admin-footer.php');
	...
	if ( !isset($wp_importers) || !isset($wp_importers[$importer]) || ! is_callable($wp_importers[$importer][2])) {
		if (! file_exists(ABSPATH . "blogAdmin/import/$importer.php"))
			wp_redirect( admin_url( 'import.php?invalid=' . $importer ) );
		include(ABSPATH . "blogAdmin/import/$importer.php");
	}
