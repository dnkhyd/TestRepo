...
//			 $('#headingDivEmailSuc').html('');
          ...
          window.location.href=appPageURL+'views/thankyou.html';
          /*
...
/****************************************************** New Sign up process Start *****************************************************/

function showSignInPopup(option)
{
	try 
	{
		//$('#mainSignDivCont').css('display', 'block');
		$('#grayOut').css('display', 'block');
		parent.$('#modalFPDivCont').css('display', 'none');
		$('#modalSignDivCont').css('display', 'block');
		$('#ifrLogin').css('display', 'block');
		var totalH = $(window).height();
		var totalW = $(window).width();
		var modX;
		var modY;
		if (totalH > $("#modalSignDivCont").height()) 
		{
			modX = (totalH - $("#modalSignDivCont").height()) / 2;
		} 
		else 
		{
			modX = 0;
			$("#modalSignDivCont").css("height", totalH);
		}
		if (totalW > $("#modalSignDivCont").width()) 
		{
			modY = (totalW - $("#modalSignDivCont").width()) / 2;
		} 
		else 
		{
			modY = 0;
			$("#modalSignDivCont").css("height", totalW);
		}
		$("#modalSignDivCont").css({"top" : modX,"left" : modY});
		var realH=modX+($("#modalSignDivCont").height()-$('#loaderFoot').height())/2;
		var realW=modY+($("#modalSignDivCont").width()-$('#loaderFootCont').width())/2;
		var isIE5Min = getIEVersionNumber();
...
function closeSigninPopup(action)
{
	...
	(document.getElementById('fname'))?document.getElementById('fname').value = "":"";
	(document.getElementById('lname'))?document.getElementById('lname').value = "":"";
	(document.getElementById('dispName'))?document.getElementById('dispName').value = "":"";
	(document.getElementById('confPwd'))?document.getElementById('confPwd').value = "":"";
	(document.getElementById('log'))?document.getElementById('log').value = "":"";
	(document.getElementById('pwd'))?document.getElementById('pwd').value = "":"";
	/*** clearing the form controls ***/
}
function submitForm(con)
{
	
	var email="";var token="";var fname="";var lname="";var nicename="";
	if(con == 'log')
	{
		document.getElementById('identifier').value = "mkh-login";
	...
	{
/*
alert(document.getElementById('identifier').value);
alert(document.getElementById('fname').value);
alert(document.getElementById('lname').value);
alert(document.getElementById('dispName').value);
alert(document.getElementById('email').value);
alert(document.getElementById('password').value);
return;
*/
		document.getElementById('identifier').value = "mkh-reg";
		email = document.getElementById('email').value;
		token = document.getElementById('password').value;
		fname = document.getElementById('fname').value;
		lname = document.getElementById('lname').value;
		nicename = document.getElementById('dispName').value;
		
		if(fname == "")
		{
			document.getElementById("errMsg").innerHTML = "Please enter your first name."; 
			document.getElementById("errMsg").style.display = "block";
			return;
		}
		else
		...
		if(lname == "")
		{
			document.getElementById("errMsg").innerHTML = "Please enter your last name."; 
			document.getElementById("errMsg").style.display = "block";
			return;
		}
		else
		...
		if(nicename == "")
		{
			document.getElementById("errMsg").innerHTML = "Please enter the display name."; 
			document.getElementById("errMsg").style.display = "block";
			return;
		}
		else
		...
		if(document.getElementById('confPwd').value != token)
		{
			document.getElementById("errMsg").innerHTML = "Password and Confirm password did not match."; 
			document.getElementById("errMsg").style.display = "block";
			return;
		}
		else
	...
	$.post("http://www.mykeralahotels.in/kerala/travel/authenticate.php",
	   {
			"email":email,
			"token":token,
			"fname":fname,
		...
		function(data)
		{
			...
			if(res.status == "ok")
			{
				if(con == "reg")
				{
					document.getElementById("errMsg").innerHTML = "You successfully got registered with My Kerala Hotels. <br/>Please activate your account by clicking on the link sent to your email Id."; 
					document.getElementById("errMsg").style.display = "block";
					...
					setTimeout("closeSigninPopup(0)",30000);
					return;
				}
				else
				{
				...
				var chk = (document.getElementById('rememberme').checked)?"checked":"false";
				//alert(chk);
				//divTag.innerHTML = '<form id="loginfrm" name="loginfrm" action="http://www.mykeralahotels.in/kerala/travel/wp-login.php" method="post" ><input type="hidden" name="redirect_to" value="http://www.mykeralahotels.in/kerala/travel/authenticate.php?con=post" /><input type="text" name="log" id="log" value="'+res.email+'" /><input type="checkbox" name="rememberme" id="rememberme" checked="'+chk+'" /><input type="password" name="pwd" id="pwd" value="'+res.pass+'" /><input type="checkbox" name="rememberme" id="rememberme" /></form>';
				divTag.innerHTML = '<form id="loginfrm" name="loginfrm" action="http://www.mykeralahotels.in/kerala/travel/wp-login.php" method="post" ><input type="hidden" name="abcd" value="abceee"><input type="hidden" name="redirect_to" value="http://www.mykeralahotels.in/kerala/travel/authenticate.php?con=post&rm='+chk+'&from=mkh&user='+res.user+'" /></form>';
				document.body.appendChild(divTag);
				document.getElementById("loginfrm").submit();
			}
			else if(res.status == "duplicate")
			{
				document.getElementById("errMsg").innerHTML = "User already exists."; 
//				document.getElementById("email").style;
				document.getElementById("errMsg").style.display = "block";
			}
			else if(res.status == "fail")
			{
				document.getElementById("errMsg").innerHTML = "Wrong email and password."; 
				document.getElementById("errMsg").style.display = "block";
			}
		}									
	);
		...
		if (str.indexOf(dot)==-1 || str.indexOf(dot)==0 || str.indexOf(dot)==lstr){
		 ...
		 if (str.indexOf(dot,(lat+2))==-1){
		   ...
		   document.getElementById("errMsg").style.display = "block";
			...
			//console.log((str.substring(ldot+1,lstr)).length+"   "+(str.substring(ldot+1,lstr)).length)
		 
		if ((str.substring(ldot+1,lstr)).length < 2 || (str.substring(ldot+1,lstr)).length > 3){
		   document.getElementById("errMsg").innerHTML = "Please enter a valid email ID.";
		   document.getElementById("errMsg").style.display = "block";
		    return false
		 }
...
function ValidateEmail(con){
	var emailID = "";
	if(con == 1)
		emailID = document.getElementById('log');
	else if(con == 2)
		emailID = document.getElementById('emailfp');
	else
		emailID = document.getElementById('email');

	if ((emailID.value==null)||(emailID.value=="")){
		document.getElementById("errMsg").innerHTML = "Please enter your email ID.";
		document.getElementById("errMsg").style.display = "block";
		emailID.focus()
		return false
	}
...
function connect(){
         ...
         FB.init({ 
            appId:'133261960044735', cookie:true, 
...
FB.Event.subscribe('auth.login', function(response) {
  ...
  // do something with response.session
		...
		//window.location="http://www.facebook.com/index.php?lh=5da43c63aed4c7082762e71e04f691b5&eu=zphZhcRhT_OcktQ84vXV8Q";	
	
}
function clearSession(redirectUrl)
{
//		alert("clearSession");
	document.getElementById("fbimgdiv").innerHTML = "";
...
//							console.log("asdfd");
						...
						else 
						{
							...
							var nicename = response.name;
							var gender = response.gender;
							var profile_lnk = response.link;
							var locale = response.locale;
							var location = (response.location)?response.location.name.split(',')[0]:"";
							var state = (response.location)?response.location.name.split(',')[1]:"";
							var ag_id = 0;
								...
								FB.getLoginStatus(function(response) 
									{
										if (response.session) 
										{
											...
											{
												//console.log(data);
												/**** submit the form to wp-login.php ****/
												var res	= eval('('+data+')');
												
												var divTag = document.createElement("div");
												divTag.id = "fbDataPost";
												divTag.style.display = "none";
												//divTag.innerHTML = '<form id="loginfrm" name="loginfrm" action="http://www.mykeralahotels.in/kerala/travel/wp-login.php" method="post" ><input type="hidden" name="redirect_to" value="http://www.mykeralahotels.in/kerala/travel/authenticate.php?con=post" /><input type="text" name="log" id="log" value="'+res.email+'" /><input type="password" name="pwd" id="pwd" value="'+res.pass+'" /></form>';
											   	divTag.innerHTML = '<form id="loginfrm" name="loginfrm" action="http://www.mykeralahotels.in/kerala/travel/wp-login.php" method="post" ><input type="hidden" name="redirect_to" value="http://www.mykeralahotels.in/kerala/travel/authenticate.php?con=post&from=fb&user='+res.user+'" /></form>';
												document.body.appendChild(divTag);
												document.getElementById("loginfrm").submit();
							...
							document.getElementById("changePwd").style.display = "none";
							$("#fbimgdiv").html('<img height="25" width="25" style="border:1px solid #333333; padding:1px;" src="http://graph.facebook.com/'+uid+'/picture"/>');
						}
					}
		);
...
/****************************************************** New Sign up process end *****************************************************/
/************************************************** forgot password pop up start **************************************************/
function showFPPopup(option)
{
	try 
	{
		//$('#mainSignDivCont').css('display', 'block');
		closeSigninPopup(0);
		parent.$('#grayOut').css('display', 'block');
		parent.$('#modalFPDivCont').css('display', 'block');
		...
		var modX;
		var modY;
		if (totalH > parent.$("#modalFPDivCont").height()) 
		{
			modX = (totalH - parent.$("#modalFPDivCont").height()) / 2;
		} 
		else 
		{
			modX = 0;
			parent.$("#modalFPDivCont").css("height", totalH);
		}
		if (totalW > parent.$("#modalFPDivCont").width()) 
		{
			modY = (totalW - parent.$("#modalFPDivCont").width()) / 2;
		} 
		else 
		{
			modY = 0;
			parent.$("#modalFPDivCont").css("height", totalW);
		}
		parent.$("#modalFPDivCont").css({"top" : modX,"left" : modY});
		var realH=modX+(parent.$("#modalFPDivCont").height()-parent.$('#loaderFoot').height())/2;
		var realW=modY+(parent.$("#modalFPDivCont").width()-parent.$('#loaderFootCont').width())/2;
		var isIE5Min = getIEVersionNumber();
		...
		var d = new Date();
		$.post(
				"http://www.mykeralahotels.in/views/forgot_pass.php?t="+d.getUTCSeconds(),
				{},
				function(data)
				{
					parent.$('#modalFPDivCont').html(data);
					...
					sucX=modX;
					sucY=modY;
					parent.document.getElementById('modalFPDivCont').style.top=sucX+'px';
					parent.document.getElementById('modalFPDivCont').style.left=sucY+100+'px';
					*/
					parent.document.getElementById('modalSignDivCont').style.display = 'none';
					parent.$('#modalFPDivCont').css('display', 'block');	
				}
...
function closeFPPopup(action)
{
	 $('#grayOut').css('display','none');
	 $('#modalFPDivCont').css('display','none');
	 $('#modalFPDivCont').html('');
}
function submitEmail()
{
	...
	{
		$.post("http://www.mykeralahotels.in/kerala/travel/authenticate.php",
			{"email":document.getElementById('emailfp').value,"identifier":"forgotPwd"},
			function(data)
			{
				var res = eval('('+data+')');
				...
				if(res.status == "ok")
				{
					document.getElementById("errMsg").innerHTML = "An email with your password is sent to <font color='black'>'"+res.email+"'.</font>";
					setTimeout("closeFPPopup(0)",30000);
				}
				else
					document.getElementById("errMsg").innerHTML = "User does not exist.";
...
function showChangePwd()
{
	try 
	{
		...
		//$('#mainSignDivCont').css('display', 'block');
		$('#grayOut').css('display', 'block');
		$('#modalCPwdDivCont').css('display', 'block');
		var totalH = $(window).height();
		var totalW = $(window).width();
		var modX;
		var modY;
		if (totalH > $("#modalCPwdDivCont").height()) 
		{
			modX = (totalH - $("#modalCPwdDivCont").height()) / 2;
		} 
		else 
		{
			modX = 0;
			$("#modalCPwdDivCont").css("height", totalH);
		}
		if (totalW > $("#modalCPwdDivCont").width()) 
		{
			modY = (totalW - $("#modalCPwdDivCont").width()) / 2;
		} 
		else 
		{
			modY = 0;
			$("#modalCPwdDivCont").css("height", totalW);
		}
		$("#modalCPwdDivCont").css({"top" : modX,"left" : modY});
		var realH=modX+($("#modalCPwdDivCont").height()-$('#loaderFoot').height())/2;
		var realW=modY+($("#modalCPwdDivCont").width()-$('#loaderFootCont').width())/2;
		var isIE5Min = getIEVersionNumber();
		...
		$.post(
				"http://www.mykeralahotels.in/views/change_pass.php",
				{},
				function(data)
				{
					$('#modalCPwdDivCont').css('display', 'block');	
					$('#modalCPwdDivCont').html(data);
...
function closeCPwdPopup(action)
{
	 $('#grayOut').css('display','none');
	 $('#modalCPwdDivCont').css('display','none');
	 $('#modalCPwdDivCont').html('');
}
function changePwd()
{
	...
	{
		document.getElementById("errMsg").innerHTML = "Please confirm your new password."; 
		document.getElementById("errMsg").style.display = "block";
		return;
	}
	else if(newPwd != confPwd)
	{
		document.getElementById("errMsg").innerHTML = "Passwords did not match."; 
		document.getElementById("errMsg").style.display = "block";
		return;
	}
	$.post("http://www.mykeralahotels.in/kerala/travel/authenticate.php",
		{"identifier":"changePwd","oldPwd":oldPwd,"newPwd":newPwd,"loggedInUser":loggedInUser},
		function(data)
		{
			var res= eval('('+data+')');
			...
			if(res.status == 'ok')
			{
				document.getElementById("errMsg").innerHTML = "Your password has been changed."; 
				document.getElementById("errMsg").style.display = "block";
				
				setTimeout("closeCPwdPopup(0)",30000);

			}
			else if(res.status == 'fail')
			{
				document.getElementById("errMsg").innerHTML = "Please check your password."; 
				document.getElementById("errMsg").style.display = "block";
			}
...
function getPageNewStreams() {
	...
	try {
		FB
				.api('/MyKerala/feed?access_token='+accessToken+'&limit='
...
function postFeed() 
{
	...
	try 
	{
		...
		//var params = "access_token="+getFBOauth()+"&message="+escape(num.substring(0, 1000));
                var params = "access_token="+accessToken+"&message="+escape(num.substring(0, 1000));		
		if(xmlhttp == null){
			alert("Your Browser does not support Javascript.Enable Javascript.");
			return false;
		}
...
function prepareShare()
{
	            ...
	            var twitURL = 'http://www.mykeralahotels.in/kerala/travel';	 
	            var shareData	=	'<div style="float: left; width: 20px;"><a class="mykr_img fb_icon" title="Share this post in Facebook"';
	            shareData		+=	' alt="Share this post in Facebook" target="_blank"';
	            shareData		+=	'href="http://www.facebook.com/share.php?u='+encodeURIComponent(twitURL)+'&t='+shareText+'"></a></div>';
	        	shareData		+=	'<div style="padding-left: 25px;"><a class="mykr_img twit_icon" title="Share this post in Twitter" ';
