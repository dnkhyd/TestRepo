...
function buildOptions($cont, $slides, els, options, o) {
	...
	//if($.browser.msie)
	//{
		//$//slides.css({position: 'absolute', top:"13px", left:"13px"});
	//}
	//else{
...
$.fn.cycle.transitions = {
	fade: function($cont, $slides, opts) {
		...
		/*if($.browser.msie)
		{
			opts.cssBefore = { top: "13px", left: "13px" };
		}
		else{*/
...
{
					console.log("Z"+(-1*(len-i+count)));
					$(opts.els[i]).css('z-index',(-1*(len-i+count)));
}
			}
			else {
				var z = $(curr).css('z-index');
					...
					console.log("Z"+(-1*(len-i+count)));	
				$el.css('z-index',(-1*(parseInt(z)+1+count)));
			}
